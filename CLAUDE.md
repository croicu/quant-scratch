...
## Pending Tasks
- **IBKR historical-data pipeline** — [tasks/ibkr_fetch_historical_spy.md](tasks/ibkr_fetch_historical_spy.md) (testing, 2026-08-01). Account approved, IB Gateway running (paper, port 4002). `IBKRIntraDay` (`src/shared/providers/ibkr.py`) implements `IntraDayProvider` using `ib_async`; validated live against SPY 7/31/2026 — 960 clean 1-min bars, real volume through pre-/after-market (unlike quant-data's Yahoo-sourced gap). Not yet wired into `day-chart` (still hardcodes `QuantDataIntraDay`); next step is a `--provider` flag or similar. No GitHub issue opened yet. Supersedes the older [tasks/ibkr_tws_extended_hours.md](tasks/ibkr_tws_extended_hours.md) brainstorm (its design decisions — connect-per-call lifecycle, `shared.sessions.infer_session` reuse — carried forward here); that file's suggestion to place the provider inside `quant-data`'s ingest instead was superseded by explicit user direction to build it here as a `quant-scratch` provider first.
