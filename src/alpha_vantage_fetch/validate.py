...
extended-hours-inclusive 1-minute intraday bars for a ticker/date from Alpha Vantage, prints a
spot-check summary, and writes them to CSV.

Not a registered CLI (no pyproject.toml entry point) -- run directly with
Settings.alpha_vantage.api_key configured in settings.local.json:

    python -m alpha_vantage_fetch.validate [TICKER] [YYYY-MM-DD]
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if settings.alpha_vantage is None:
            raise AppError("settings.local.json needs an 'alphaVantage' section with 'apiKey' set -- see docs/PROTOCOL.md.")

        provider = AlphaVantageIntraDay(api_key=settings.alpha_vantage.api_key)
        bars = provider.fetch_bars(ticker, target_date)

        print(_spot_check(bars))

        csv_path = Path(".") / f"{ticker.upper()}_{target_date.isoformat()}_alpha_vantage_data.csv"
        csv_path.write_text(bars_to_csv(bars), encoding="utf-8", newline="")
        print(f"alpha-vantage-fetch-validate: wrote {csv_path}")
        return 0
    except AppError as error:
        print(f"alpha-vantage-fetch-validate: error: {error}", file=sys.stderr)
        return 1
    finally:
